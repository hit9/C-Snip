...
struct buf *buf_new(const char *s) {
    ...
    if (buf != NULL) {
        ...
        if (s != NULL) {
            if (buf_puts(buf, s) != BUF_OK) return NULL;
        }
    }
    return buf;
}
...
int buf_grow(struct buf *buf, size_t cap) {
    ...
    if (cap > BUF_CAP_MAX) return BUF_ENOMEM;

    if (cap <= buf->cap) return BUF_OK;

    size_t unit = buf->cap;

    if (unit > BUF_UNIT_MAX) unit = BUF_UNIT_MAX;

    if (unit < BUF_UNIT_MIN) unit = BUF_UNIT_MIN;
    ...
    if (data == NULL) return BUF_ENOMEM;
...
char *buf_str(struct buf *buf) {
    ...
    if (buf->len < buf->cap && buf->data[buf->len] == '\0') return buf->data;

    if (buf->len + 1 <= buf->cap || buf_grow(buf, buf->len + 1) == BUF_OK) {
        buf->data[buf->len] = '\0';
        return buf->data;
    }
...
int buf_sprintf(struct buf *buf, const char *fmt, ...) {
    ...
    if (buf->len >= buf->cap && buf_grow(buf, buf->len + 1) != BUF_OK)
        return BUF_ENOMEM;
    ...
    if (num < 0) return BUF_EFAILED;

    size_t size = (size_t)num;

    if (size >= buf->cap - buf->len) {
        if (buf_grow(buf, buf->len + size + 1) != BUF_OK) return BUF_ENOMEM;
        va_start(ap, fmt);
        num = vsnprintf(buf->data + buf->len, buf->cap - buf->len, fmt, ap);
        va_end(ap);
    }
